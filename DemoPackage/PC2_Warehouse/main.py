...
class MainLauncher:
    ...
    def _launch(self, script):
        """Launch a module - handles both .py and .exe modes."""
        if not self.db_connected:
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                # ─── RUNNING AS .EXE ───
                exe_map = {
                    'apps/shed_app.py': 'ShedApp.exe',
                    'apps/container_app.py': 'ContainerApp.exe',
                    'apps/box_app.py': 'BoxApp.exe',
                    'apps/trip_card_app.py': 'TripCardApp.exe',
                    'apps/gate_app.py': 'GateApp.exe',
                    'apps/warehouse_app.py': 'WarehouseApp.exe',
                    'apps/uhf_writer_app.py': 'UHFWriterApp.exe',
                    'apps/dashboard_app.py': 'DashboardApp.exe',
                }
                
                exe_name = exe_map.get(script)
                if not exe_name:
                    messagebox.showwarning("Not Available",
                        f"Module not available: {script}")
                    return
                ...
                args = [
                    exe_path,
                    "--from-launcher",
                    "--user", Session.get_username() or "unknown",
                    "--role", Session.get_role() or "ADMIN",
                    "--name", Session.get_full_name() or "User"
                ]
                
                subprocess.Popen(args)
                print(f"[Launcher] Started: {exe_name}")
